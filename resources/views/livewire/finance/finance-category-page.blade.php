        ...
        <div class="grid md:grid-cols-[1fr_2fr] gap-3">
            <div>
                ...
                <div class="relative p-4 mb-2 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                    ...
                    <!-- Modal body -->
                    <form wire:submit="{{ $editId ? 'updateCategory' : 'createCategory' }}" action="#" method="POST">
                        <div class="gap-4 overflow-y-auto max-h-[680px] relative z-10">
                            <div class=" mb-4">
                                <label class="block py-2 text-sm font-medium text-gray-900 dark:text-white">Nama Kategori</label>
                                <input wire:model="category_name" type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required="">
                                <div>
                                    @error('category_name') <span class="text-red-700">{{ $message }}</span> @enderror 
                                </div>
                            </div>
                            <div class="sm:col-span-2 mb-4">
                                <label class="block py-2 text-sm font-medium text-gray-900 dark:text-white">Deskripsi Kategori</label>
                                <textarea wire:model="category_description" rows="9" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Write product description here"></textarea>                    
                            </div>
                            <div class="sm:col-span-2 mb-4">
                                    ...
                                    @foreach ($categories as $c)
                                        <tr class="border-b dark:border-gray-700">
                                            ...
                                            <th scope="row" class="px-3 font-medium text-gray-900 whitespace-nowrap dark:text-white py-3">
                                                ...
                                                @endif
                                                <br><span class="text-xs text-white">{{ $c->category_description }}</span>
                                            </th>
                                            <td class="px-3">
                                                <button wire:click="editCategory({{ $c->id }})" class="text-white bg-warning box-border border border-transparent hover:bg-warning-strong focus:ring-4 focus:ring-success-medium shadow-xs font-medium leading-5 rounded-md text-xs px-1.5 py-1 focus:outline-none">
